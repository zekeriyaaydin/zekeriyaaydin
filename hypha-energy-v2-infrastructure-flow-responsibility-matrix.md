# My Thoughts about v2 Infrastructure
...
* **Zek:** Managing the **Infrastructure Foundation** (Steps 0-3) and **Platform Services** (Steps 8-10). Coordinating with **R&C and Grexx** at the physical edge and handling the data flow (Telemetry, Price Oracles, and Ingestion) and member-facing services (Billing/Identity).
* **Vlad:** Managing the **Value & Control Engine** (Steps 4-7). Picking up the data from the DB to drive the **VPP Fair-Split**, **EMS Control Logic**, and **On-Chain Settlement**.
...
| **8** | **Billing/Invoicing** | **Zek** | **C# / PDF Engine** | AWS Lambda / S3 |
| **9** | **Identity / Auth** | **Zek** | **Clerk / Azure AD B2C** | AWS Cognito |
| **10** | **Notifications** | **Zek** | **Azure Communication Svc** | AWS SNS / SES |
...
* **Billing Trigger:** The Billing service generates member statements by reconciling `interval_readings` against Vlad's `settlement_batches`.

### Database Schema Specs
* **raw_readings (TimescaleDB):** 10-second high-fidelity data. *Retention: 90 days.*
* **interval_readings (TimescaleDB):** 15-minute
